...
frappe.ui.form.on("Asset", {
	...
	refresh: function (frm) {
		...
		if (frm.doc.docstatus == 0) {
			...
			if (frm.doc.is_composite_asset) {
				frappe.call({
					method: "assets.assets.doctype.asset.asset.has_active_capitalization",
					args: {
						asset: frm.doc.name,
					},
					callback: function (r) {
	...
	setup_chart_and_depr_schedule_view: async function (frm) {
		...
		if (frm.doc.calculate_depreciation) {
			...
			let asset_depr_schedule_doc = (
				await frappe.call(
					"assets.assets.doctype.asset_depreciation_schedule.asset_depreciation_schedule.get_asset_depr_schedule_doc",
					{
						asset_name: frm.doc.name,
						status: "Active",
	...
	set_finance_book: function (frm) {
		frappe.call({
			method: "assets.assets.doctype.asset.asset.get_item_details",
			args: {
	...
	make_sales_invoice: function (frm) {
		frappe.call({
			...
			method: "assets.assets.doctype.asset.asset.make_sales_invoice",
			callback: function (r) {
				var doclist = frappe.model.sync(r.message);
				frappe.set_route("Form", doclist[0].doctype, doclist[0].name);
			},
...
frappe.ui.form.on("Asset Finance Book", {
	...
	depreciation_start_date: function (frm, cdt, cdn) {
		...
		if (
			...
			frappe.msgprint(
				__("Depreciation Posting Date cannot be before Available-for-use Date")
			);
...
erpnext.asset.restore_asset = function (frm) {
	frappe.confirm(__("Do you really want to restore this scrapped asset?"), function () {
		frappe.call({
			...
			method: "assets.assets.doctype.asset.depreciation.restore_asset",
			callback: function (r) {
				cur_frm.reload_doc();
			},
		});
